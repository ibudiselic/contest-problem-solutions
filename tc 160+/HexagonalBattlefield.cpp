...
struct State {
    int r, c, here, next;
    State(int r_, int c_, int here_, int next_): r(r_), c(c_), here(here_), next(next_) {}
};
...
long long lastrow(int r, int c, int here) {
    if (c == (int)A[r].size()) {
        return 1;
    }
    
    const State s(r, c, here, 0);
    if (dp.count(s)) {
        return dp[s];
    }
    long long &ret = dp[s];
...
long long go(int r, int c, int here, int next) {
    if (r+1 == (int)A.size()) {
        assert(c == 0);
