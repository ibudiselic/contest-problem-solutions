...
string mul(const string &a, char d) {
    int c = 0;
    string res;
    for (int i=0; i<(int)a.size(); ++i) {
        int next = (a[a.size()-1-i]-'0') * (d-'0') + c;
        res += char(next%10 + '0');
        c = next/10;
    }
    if (c) {
...
string add(string a, string b) {
    ...
    a = string(sz-a.size(), '0') + a;
    b = string(sz-b.size(), '0') + b;
    string res;
    int c = 0;
    for (int i=0; i<sz; ++i) {
        int d = a[a.size()-1-i]-'0' + b[b.size()-1-i]-'0' + c;
        res += char(d%10 + '0');
        c = (d>=10);
    }
    if (c) {
...
string calc(string x, string y, int cnt) {
    bool have_same = false;
    bool diff = false;
    for (int i=0; i<(int)x.size() && cnt; ++i) {
        if (x[i] == y[i]) {
            have_same = true;
        } else {
            if (diff) {
...
class DigitsSwap {
	...
	string maximalProduct(string x, string y, int swaps) {
		string a = calc(x, y, swaps);
        string b = calc(y, x, swaps);
