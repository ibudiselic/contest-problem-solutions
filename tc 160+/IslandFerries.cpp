...
bool canTravel[40][40][10]; // [i][j][k] travel with service 'k' from island 'i' to isnald 'j'
int ticketCost[40][10]; // [i][j] cost of a ticket for service 'j' on island 'i'
int best[40]; // distance
int bc[1<<10];
bool done[1<<16];

int encode(int island, int ticketMask) {
	// 10lsb for ticketMask, 6msb for island
	return (island<<10) + ticketMask;
}
void decode(int code, int &island, int &ticketMask) {
	ticketMask = code & 0x3ff;
	island = (code>>10) & 0x3f;
}
class IslandFerries {
	public:
	vector <int> costs(vector <string> legs, vector <string> prices) {
		const int nServices = legs.size();
		const int nIslands = prices.size();

		memset(canTravel, 0, sizeof canTravel);
		...
		bc[0] = 0;
		for (int i=0; i<(1<<nServices); ++i)
			bc[i] = bc[i>>1] + (i&1);

		memset(best, 0xff, sizeof best);

		priority_queue< pair<int, int>, vector< pair<int, int> >, greater< pair<int, int> > > PQ;
		PQ.push(make_pair(0, encode(0, 0)));
		memset(done, 0, sizeof done);
		
		while (!PQ.empty()) {
			int cost = PQ.top().first;
			int code = PQ.top().second;
			PQ.pop();
			...
			int island=0, ticketMask=0;
			decode(code, island, ticketMask);

			if (ticketMask == 0)
				best[island] = cost;

			if (bc[ticketMask] < 3)
				for (int service=0; service<nServices; ++service)
					if ((ticketMask & (1<<service)) == 0) {
						int nextCode = encode(island, ticketMask | (1<<service));
						if (!done[nextCode])
							PQ.push(make_pair(cost + ticketCost[island][service], nextCode));
					}

			for (int target=0; target<nIslands; ++target)
				for (int service=0; service<nServices; ++service)
					if ((ticketMask & (1<<service)) && canTravel[island][target][service]) {
						int nextCode = encode(target, ticketMask ^ (1<<service));
						if (!done[nextCode])
							PQ.push(make_pair(cost, nextCode));
					}
		}

		return vector<int> (best+1, best+nIslands);
	}
