...
class Subway2 {
	...
	double minTime(int length, int maxAcceleration, int maxVelocity) {
        double t = sqrt(double(length) / maxAcceleration);
        if (t*maxAcceleration - 1e-9 > maxVelocity) {
            double tt = double(maxVelocity)/maxAcceleration;
            return 2*tt + (length-tt*maxVelocity)/maxVelocity;
        } else {
            return 2*t;
        }
	}
