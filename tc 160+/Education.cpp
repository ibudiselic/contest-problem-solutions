...
class Education {
	...
	int minimize(string desire, vector <int> tests) {
		int need=0;
		switch (desire[0]) {
			case 'A': need = 90; break;
			case 'B': need = 80; break;
			case 'C': need = 70; break;
			case 'D': need = 60; break;
		}
		const int sol = max(0, need*((int)tests.size()+1) - accumulate(tests.begin(), tests.end(), 0));
