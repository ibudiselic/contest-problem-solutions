...
class ProgressBar {
	...
	string showProgress(vector <int> taskTimes, int tasksCompleted) {
		int t = accumulate(taskTimes.begin(), taskTimes.end(), 0);
		int tc = accumulate(taskTimes.begin(), taskTimes.begin()+tasksCompleted, 0);

		return string((int)(double(tc)/t*20), '#')+string(20-(int)(double(tc)/t*20), '.');
	}
