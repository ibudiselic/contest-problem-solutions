...
class Highscore {
	...
	int getRank(vector <int> scores, int newscore, int places) {
		...
		p.push_back(1);
		int pos = 1;
		for (int i=1; i<min(places, (int)scores.size()); ++i) {
			if (scores[i] != scores[i-1])
				pos = i+1;
			p.push_back(pos);
		}

		for (int i=0; i<min(places, (int)scores.size()); ++i)
			if (scores[i] < newscore)
				return i ? (scores[i-1]==newscore ? p[i-1] : p[i]) : 1;

		if (places > (int)scores.size())
			return (scores.size()>0 && newscore==scores.back() ? p.back() : (int)scores.size()+1);
