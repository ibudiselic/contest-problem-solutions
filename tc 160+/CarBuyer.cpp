...
//"<PURCHASE COST> <TAX> <FUEL EFFICIENCY>"
// FUEL PRICE * DISTANCE DRIVEN / FUEL EFFICIENCY
class CarBuyer {
	public:
	double lowestCost(vector <string> cars, int fuelPrice, int annualDistance, int years) {
		...
		for (int i=0; i<(int)cars.size(); ++i) {
			int p, t, fe;
			sscanf(cars[i].c_str(), "%d %d %d", &p, &t, &fe);

			sol = min(sol, p + years*t + fuelPrice*annualDistance*years*1.0/fe);
		}
