...
bool canMake(long long cnt, int k, const vector<int> &C) {
	int col = 0;
	long long row = 0;
	int i = 0;

	while (col<k && i<(int)C.size()) {
		while (i<(int)C.size()) {
			row += min((long long)C[i], cnt);
			++i;
			if (row >= cnt) {
				row %= cnt;
				++col;
...
class WorldPeace {
	...
	long long numGroups(int k, vector <int> countries) {
		long long lo = 1, hi = 25000000000LL;

		while (lo < hi) {
			long long mid = lo + (hi-lo+1)/2;

			if (canMake(mid, k, countries))
				lo = mid;
			else
				hi = mid-1;
		}
