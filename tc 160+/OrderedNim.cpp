...
class OrderedNim {
	public:
	string winner(vector <int> layout, int p=-1, bool nextStarterWins=false) {
        if (p == -1) {
            p = (int)layout.size() - 1;
        }
        if (p == 0) {
            if (layout[p] == 1) {
                return nextStarterWins ? "Bob" : "Alice";
            } else {
                return "Alice"; // can start on next or make Bob start on next
            }
        }
        if (layout[p] == 1) {
            return winner(layout, p-1, !nextStarterWins);
        } else {
            return winner(layout, p-1, true);
        }
	}
