...
class PyramidOfCubes {
	public:
	double surface(long long K) {
		long long n;
		for (n=1; n*(n+1)*(2*n+1)/6 < K; ++n)
			;

		double sol = 2.0*min(n*n, K); /* top and bottom */

		long long t;
		for (t=0; (n*(n+1)*(2*n+1)/6 - t*(t+1)*(2*t+1)/6 > K); ++t)
			;

		sol += 4*(n*(n+1)/2) - 4*(t*(t+1)/2);
		K -= n*(n+1)*(2*n+1)/6 - t*(t+1)*(2*t+1)/6;
		n = t;
		while (K > 0) {
			sol += 2*((K+n-1)/n) + 2*min(n, K);
			K -= n*n;
			--n;
		}
		return sol;
