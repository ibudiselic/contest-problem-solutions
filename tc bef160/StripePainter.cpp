...
class StripePainter {
	...
	int minStrokes(string stripes) {
		...
		for (int left=(int)stripes.size(); left>0; --left)
			for (int size=1; size<=(int)stripes.size()-left+1; ++size)
				for (int color=0; color<27; ++color)
					if (color == stripes[left-1]-'A')
						din[left][size][color] = din[left+1][size-1][color];
					else
						for (int i=1; i<=size; ++i) {
							int t = 1+din[left+1][i-1][stripes[left-1]-'A']+din[left+i][size-i][color];
							if (din[left][size][color]==0 || t<din[left][size][color])
								din[left][size][color] = t;
						}

		return din[1][stripes.size()][26];
	}
