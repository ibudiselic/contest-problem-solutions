...
struct TournamentTree {
    ...
    // -1 if no value is < val
    int get_leftmost_lt(int l, int r, int val) const {
        this->l = l;
        this->r = r;
        return get_leftmost_lt(1, 0, n, val);
    }

    int get_leftmost_lt(int node, int a, int b, int val) const {
        if (b<=l || a>=r) return -1;
        if (l<=a && b<=r) {
            if (T[node]>=val) return -1;
            if (a+1 == b) return a;
        }
        const int mid = a + (b-a)/2;
        int ret = get_leftmost_lt(2*node, a, mid, val);
        return ret!=-1 ? ret : get_leftmost_lt(2*node+1, mid, b, val);
    }
};
const int MAXN = 100000;
int idx[2*MAXN];
vector<int> L;
bool can(int i, int len, const vector<int> &invSA, const TournamentTree &T) {
    int n = invSA.size();
    i = invSA[i];
    int r = T.get_leftmost_lt(i+1, n, len) - 1;
    if (r < 0) {
        r = n-1;
    }
    int l = L[r];
    return l!=-1 && (l>=i || T.get_min(l+1, i+1)>=len);
}
int main() {
    int n, k;
    cin >> n >> k;
    ...
    for (int i=0; i<(int)S.size(); ++i) {
        ...
        int j = starts_at[at+1] - 1;
        int hi = j - i;
        int lo = last;
        while (lo < hi) {
            int mid = lo + (hi-lo+1)/2;
            if (can(i, mid, invSA, T)) {
                lo = mid;
            } else {
                hi = mid - 1;
            }
